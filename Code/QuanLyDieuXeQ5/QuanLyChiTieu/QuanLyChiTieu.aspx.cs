...
public partial class QuanLyChiTieu_QuanLyChiTieu : System.Web.UI.Page
{
    ...
    {
        string sql = @"select count(idChiKhac) from (
 select ck.*,HoTen from tb_ChiKhac ck, tb_NguoiDung nd where ck.idNguoiDung = nd.idNguoiDung) as T where '1'='1'";
        if (sTuNgay != "")
 ...
 select ck.*,HoTen from tb_ChiKhac ck, tb_NguoiDung nd where ck.idNguoiDung = nd.idNguoiDung) as T where 1 = 1
        ...
        for (int i = 0; i < table.Rows.Count; i++)
        {
            ...
            html += "       <td style='text-align:center;vertical-align: inherit;font-size:20px;white-space: nowrap;'>" + DateTime.Parse(table.Rows[i]["NgayChi"].ToString()).ToString("dd/MM/yyyy") + "</td>";
            html += "       <td style='text-align:center;vertical-align: inherit;font-size:20px;white-space: nowrap;'>" + table.Rows[i]["NoiDung"].ToString() + "</td>";
            if (table.Rows[i]["SoTien"].ToString() != "")
                html += "       <td style='text-align:center;vertical-align: inherit;font-size:20px;white-space: nowrap;'>" + double.Parse(table.Rows[i]["SoTien"].ToString()).ToString("#,##").Replace(",", ".") + "</td>";
            else
                html += "   <td style='text-align:center;vertical-align: inherit;font-size:20px;white-space: nowrap;'></td>";
            html += "       <td  style='text-align:center;vertical-align: inherit;font-size:20px;white-space: nowrap;'>" + table.Rows[i]["HoTen"].ToString() + "</td>";
            ...
            if (mQuyen.ToUpper() == "ADMIN")
            {
                ...
                html += "           <a href='#' onclick='window.location=\"QuanLyChiTieu-CapNhat.aspx?Page=" + Page.ToString() + "&idChiTieu=" + table.Rows[i]["idChiKhac"].ToString() + "\"'><i class='fa fa-edit'></i></a>";
                html += "           <a href='#' onclick='DeleteChiTieu(\"" + table.Rows[i]["idChiKhac"].ToString() + "\")'> <i class='fa fa-trash'></i></a>";
