...
public partial class QuanLyDonHang_QuanLyDonHang_CapNhat : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            sIdPhanHangLenXe = StaticData.ValidParameter(Request.QueryString["idPhanHangLenXe"].Trim());
        }
        catch { }
        try
        {
    ...
    private void TaoMaDonHang()
    {
        string MaNhapMua = "";
        string sql = "select isnull(max(idPhanHangLenXe),0)+1 as 'MaNhapMua' from tb_PhanHangLenXe";
        DataTable table = Connect.GetTable(sql);
        MaNhapMua = table.Rows[0]["MaNhapMua"].ToString();
        string Ngay = DateTime.Now.ToString("ddMMyyyy");
        // txtMaDonHang.DataSource = Connect.GetTable(sql);
        txtMaDonHang.Value = Ngay  + MaNhapMua + "";
    }
    ...
    private void LoadThongTinDonHang()
    {
        ...
        if (sIdPhanHangLenXe != "")
        {
            ...
            if (table.Rows.Count > 0)
            {
                ...
                string sql1 = @"select ct.idChiTietDonHang,hh.MaHangHoa,TenKhachHang,TenChiNhanh,kh.DiaChi,SoLuong , TenHangHoa, NguoiNhanTra from 
tb_ChiTietDonHang ct, tb_DonHang dh, tb_HangHoa hh, tb_KhachHang kh,tb_ChiNhanh cn where 
                ct.idDonHang = dh.idDonHang and ct.idHangHoa = hh.idHangHoa and dh.idKhachHang = kh.idKhachHang and dh.idChiNhanhNhan = cn.idChiNhanh
				 and IdPhanHangLenXe = '" + sIdPhanHangLenXe + "'";
    ...
    protected void loadSP(DataTable data)
    {
        if (data.Rows.Count > 0)
        {
            ...
            for (int i = 0; i < data.Rows.Count; i++)
            {
                ...
                slistSP += data.Rows[i]["idChiTietDonHang"] + "-" + data.Rows[i]["MaHangHoa"] + "-" + data.Rows[i]["TenHangHoa"] + "-" + data.Rows[i]["SoLuong"];
                html += "<tr id='tr_" + data.Rows[i]["idChiTietDonHang"] + "'>";
                html += "<td>" + (i + 1) + "</td>";
                html += "     <td style='text-align:center;vertical-align: inherit;'>" + data.Rows[i]["MaHangHoa"] + "</td>";
                html += "     <td style='text-align:center;vertical-align: inherit;'>" + data.Rows[i]["TenKhachHang"] + "</td>";
                html += "     <td style='text-align:center;vertical-align: inherit;'>" + data.Rows[i]["TenChiNhanh"] + "</td>";
                html += "     <td style='text-align:center;vertical-align: inherit;'>" + data.Rows[i]["DiaChi"] + "</td>";
                ...
                if (data.Rows[i]["NguoiNhanTra"] == "True")
                html += "     <td style='text-align:center;vertical-align: inherit;'> Người nhận trả</td>";
                else
                    html += "     <td style='text-align:center;vertical-align: inherit;'> Người gửi trả</td>";
      ...
      //  GoiDichVu = slGoiDichVu.Value.Trim();
        ...
        if (sIdPhanHangLenXe == "")
        {
           // string MaDonHang1 = MyStaticData.TaoMaDonHang();
            string sqlInsertDH = "insert into tb_PhanHangLenXe(MaTuyen,NgayLap,idXe,idNguoiDung)";
            ...
            if (ktInsertKH)
            {
                string idPhanHangLenXe = StaticData.getField("tb_PhanHangLenXe", "idPhanHangLenXe", "MaTuyen", MaTuyen);
                ...
                sqlUpdateKhachHang += " IDPhanHangLenXe = '" + idPhanHangLenXe + "'";
