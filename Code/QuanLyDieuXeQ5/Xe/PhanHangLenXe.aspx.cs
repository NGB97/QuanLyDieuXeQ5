...
public partial class QuanLyDonHang_QuanLyDonHang : System.Web.UI.Page
{
    ...
    {
        string sql = "select count(idPhanHangLenXe) from tb_PhanHangLenXe where '1'='1'";
        if (sTuNgay != "")
    ...
    private void LoadDonHang()
    {
        ...
        sql += @"select * from
            (
	            SELECT ROW_NUMBER() OVER ( ORDER BY idPhanHangLenXe desc )
                AS RowNumber ,phlx.*,x.BienSoXe,'HoTen' = (select HoTen from tb_NguoiDung where tb_NguoiDung.IDNguoiDung = x.IDNguoiDung),
                'TenNguoiDung'=(select HoTen from tb_NguoiDung where idNguoiDung = phlx.idNguoiDung )
                FROM tb_PhanHangLenXe phlx, tb_Xe x
     ...
     //   html += "               <td style='text-align:center;vertical-align: inherit;'><b>" + TongTienCuoc.ToString("N0").Replace(",", ".") + "</b></td>";
      ...
      //  html += "               <td style='text-align:center;vertical-align: inherit;'><b>" + (TongTienHang + TongTienCuoc + TongPhuPhi).ToString("N0").Replace(",", ".") + "</b></td>";
        ...
        for (int i = 0; i < table.Rows.Count; i++)
        {
            ...
            string idDonHang = table.Rows[i]["idPhanHangLenXe"].ToString();
          ...
          //  html += "       <td style='text-align:center;vertical-align: inherit;'>" + table.Rows[i]["MaTuyen"].ToString() + "</td>";
            ...
            html += "       <td style='text-align:center;vertical-align: inherit;'>" + table.Rows[i]["TenNguoiDung"].ToString() + "</td>";
         ...
         //   html += "                              <th class='th' style='background-color: #1c9c13; font-size: 15px; color: white;'>Chức Năng</th>";
            ...
            string sqlCTDH = "select * from tb_ChiTietDonHang where idPhanHangLenXe='" + table.Rows[i]["idPhanHangLenXe"].ToString() + "'";
            DataTable tbCTDH = Connect.GetTable(sqlCTDH);
           ...
           // html += "                       <tbody id='danhSachSPChon' runat='server'></tbody>";
            ...
            html += "           <a class='btn btn-default' title='In biên nhận' onclick='PrinfPhanHangLenXe(\"" + table.Rows[i]["idPhanHangLenXe"].ToString() + "\")'><i class='fa fa-print'></i><sapn> In biên nhận</span></a>";
            ...
            else
            {
                ...
                //{
                html += "       <a class='btn btn-primary' style='cursor:pointer' title='Sửa đơn hàng' onclick='window.location=\"PhanHangLenXe-CapNhat.aspx?Page=" + Page.ToString() + "&idPhanHangLenXe=" + table.Rows[i]["idPhanHangLenXe"].ToString() + "\"'><i class='fa fa-edit'></i><span> Cập nhật</span></a>";
                //}
            }
